...
public:
 ...
 //* Subclass of BankAccount which implements the deposit, withdraw, and checkBalance functions for a Savings Account
...
public:
    ...
    void deposit(const double amount) override {
        if (amount <= 0) {
            wcout << "Invalid deposit amount. Please enter a value." << endl;
        } else {
            balance += amount;
...
public:
    ...
    bool withdraw(const double amount) override {
        ...
        if (balance - amount < 0) {
            wcout << "Insufficient balance" << endl;
            return false;
        }
...
double getValidAmount() {
    double amount;
    bool isValid = false;  // A flag to control the loop
    while (!isValid) {
        wcin >> amount;
        if (wcin.fail() || amount < 0) {
            ...
            isValid = true;  // Input is valid, terminate the loop
        }
    }
    return amount;
}
...
int main() {
    ...
    do {
        ...
        switch (choice) {
            ...
            case 3:
                wcout << "Exiting the system. Thank You For Using!" << endl;
                break;
            default:
                wcout << "Invalid option. Please Input A Valid Option." << endl;
                pauseScreen();
                continue;
        }
